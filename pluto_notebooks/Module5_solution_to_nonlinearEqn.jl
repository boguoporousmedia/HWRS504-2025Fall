...
In general, we wish to find roots, or zeros, of the general nonlinear equation
...
begin
    ...
    local p = plot(xs, ys, xlabel="x", ylabel="F(x)", legend=false)
    hline!(p, [0])                       # x-axis
    scatter!(p, [cbrt(25)], [F(cbrt(25))], markershape=:+, markersize=8)  # orange 'X' at x=3
    p
end
...
To estimate the root within error tol, use an algorithm in the following Pseudocode:
...
✓ A more systematic (improved) and very popular method based on truncated Taylor series.
...
local img = LocalResource("./figs/mod5_fixed_pt_1.png", :width => "600px")

# ╔═╡ 2a8486a4-070c-4c52-8cf2-38061da43561
local img = LocalResource("./figs/mod5_fixed_pt_2.png", :width => "600px")
...
```math
  \Rightarrow \;\; \text{Excellent good behavior.}
```
